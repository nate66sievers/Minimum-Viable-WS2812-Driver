...
#define NUM_LEDS 3
#define LED_BUFFER_SIZE (NUM_LEDS * 24)
...
void WS2812_SetColor(uint8_t ledIndex, uint8_t red, uint8_t green, uint8_t blue) {
	uint32_t ledBitPosition = ledIndex*24;
    for (int i = 0; i < 8; i++) {
        ledBuffer[ledBitPosition + i] = (green & (1 << (7 - i))) ? 0xF8 : 0xC0;
        ledBuffer[ledBitPosition + 8 + i] = (red & (1 << (7 - i))) ? 0xF8 : 0xC0;
        ledBuffer[ledBitPosition + 16 + i] = (blue & (1 << (7 - i))) ? 0xF8 : 0xC0;
    }
}
